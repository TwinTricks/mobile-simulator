...
// --- User overrides ---
// Optional: user can drop `<name>.svg` files into `icons/ui/` and the
// extension will swap them in over the built-in equivalent. The file
// must be a complete <svg> element using currentColor.
const USER_OVERRIDE_DIR = 'icons/ui/';
const overrideCache = new Map();

async function tryFetchOverride(name) {
  if (overrideCache.has(name)) return overrideCache.get(name);
  try {
    const rt = globalThis.chrome?.runtime || globalThis.browser?.runtime;
    if (!rt?.getURL) { overrideCache.set(name, null); return null; }
    const url = rt.getURL(`${USER_OVERRIDE_DIR}${name}.svg`);
    const res = await fetch(url);
    if (!res.ok) { overrideCache.set(name, null); return null; }
    const text = await res.text();
    // Add the icon-svg class so existing CSS sizing rules apply.
    const patched = text.replace(/<svg\b([^>]*)>/i, (m, attrs) => {
      if (/class\s*=/.test(attrs)) {
        return `<svg${attrs.replace(/class\s*=\s*("|')([^"']*)\1/, (_, q, c) => `class=${q}${c} icon-svg${q}`)}>`;
      }
      return `<svg${attrs} class="icon-svg">`;
    });
    overrideCache.set(name, patched);
    return patched;
  } catch {
    overrideCache.set(name, null);
    return null;
  }
}

// Async pass — walks the DOM and replaces any icon that has an override
// file on disk. Safe to call repeatedly. Run after applyIcons() to avoid
// a flash of un-styled content for built-ins.
export async function applyIconOverrides(root = document) {
  const nodes = root.querySelectorAll('[data-icon]');
  const seen = new Set();
  const tasks = [];
  nodes.forEach((el) => {
    const name = el.getAttribute('data-icon');
    if (!name) return;
    if (el.dataset.iconOverrideChecked === '1') return;
    seen.add(name);
    tasks.push(
      tryFetchOverride(name).then((svg) => {
        el.dataset.iconOverrideChecked = '1';
        if (svg) {
          el.innerHTML = svg;
          el.dataset.iconLoaded = '1';
        }
